...
export class Jobseekers implements OnInit{
    ...
    ngOnInit() {
       this.jobForm =  this.formBuilder.group({
            ...
            ssn: new FormControl('',Validators.compose([Validators.required, Validators.pattern('[0-9]{9}')])),
   ...
   onSubmit(){
       ...
       this.jobService.saveApplication(jobApp).subscribe(data => {
           this.blocked = false;
           this.messageService.add({severity:'success', summary:'Success', detail:'Application is saved !!'});
       }, error => {
           this.blocked = false;
